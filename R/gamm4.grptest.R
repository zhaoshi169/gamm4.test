...
#'sigma1 <- 0.3
#'sigma2 <- 0.2
#'sigma.noise1 <- sigma.noise2 <- 0.1
#'f1 <- function(u)sin(2*pi*u)
#'f2 <- function(u)sin(2*pi*u)+u/3
#'N1 <- m1*n1
#'N2 <- m2*n2
#'x11 <- runif(N1,0,1)
...
#'y3 <- f1(x21) + b2+ rnorm(N2,0,sigma.noise2)
#'dat <- data.frame(rbind(cbind(id1, x11,y1,1), cbind(id2, x21, y2,2)))
...
#'plot(testout)
#'
#'dat0 <- data.frame(rbind(cbind(id1, x11,y1,1), cbind(id2, x21, y3,2)))
...
#'test.spline0
#'plot(test.spline0, test.statistic = TRUE)
#'plot(test.spline0)
...
#'child.repb <- outchild1016[(outchild1016$RACE==2),]
#'child.reptest1 <- gamm4.grptest(WEIGHT~s(age),random=~(1|SID),test=~SEX,data=child.repb,parallel=TRUE)
#'child.reptest1
#'plot(child.reptest1)
#'plot(child.reptest1,test.statistic=TRUE)
#'
#'child.reptest2 <- gamm4.grptest(HEIGHT~t2(age,WEIGHT),random=~(1|SID),test=~SEX,data=child.repb,parallel=TRUE)
